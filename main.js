...
class ProgressTracker {
    ...
    start(message) {
        this.notice = new obsidian_1.Notice(`${message} (0/${this.total})`, 0);
    }
    update() {
        this.processed++;
        if (this.notice) {
 ...
 */
class BulkOperationModal extends obsidian_1.Modal {
    constructor(app, files, onConfirm, onCancel) {
        super(app);
        this.files = files;
...
class LogseqNamespaceInjector extends obsidian_1.Plugin {
    onload() {
    ...
    validateVaultState() {
        var _a;
        if (!((_a = this.app) === null || _a === void 0 ? void 0 : _a.vault)) {
            new obsidian_1.Notice('❌ Vault not available. Please try again.');
            return false;
        }
        if (!this.app.vault.adapter) {
            new obsidian_1.Notice('❌ Vault adapter not ready. Please try again.');
            return false;
        }
        return true;
    }
    ...
    shouldExcludeFile(file) {
        ...
        return this.settings.excludePatterns.some(pattern => filePath.includes(pattern.toLowerCase()));
    }
    /**
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (plannedChanges.length === 0) {
                    new obsidian_1.Notice('No files need namespace injection.');
                    return;
                }
                // Show preview
                ...
                new obsidian_1.Notice(`Preview: ${plannedChanges.length} files will be updated:\n\n${previewText}${moreText}`, 10000);
            }
            catch (error) {
                console.error('Error during preview:', error);
                new obsidian_1.Notice('❌ Error generating preview. Check console for details.');
            }
        });
    }
    /**
    ...
    getFilesToProcess() {
        ...
        return files.filter(file => {
            ...
            if (!((_a = file.parent) === null || _a === void 0 ? void 0 : _a.path))
                return false;
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            for (const file of files) {
                try {
                    ...
                    throw new Error(`Backup failed for ${file.path}: ${(error === null || error === void 0 ? void 0 : error.message) || 'Unknown error'}`);
                }
            }
            return backups;
        });
    ...
    restoreFromBackups(backups) {
        return __awaiter(this, void 0, void 0, function* () {
            for (const backup of backups) {
                try {
                    yield this.app.vault.modify(backup.file, backup.originalContent);
                }
                catch (error) {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                if (filesToProcess.length === 0) {
                    new obsidian_1.Notice('No files found to process.');
                    return;
                }
                ...
                if (plannedChanges.length === 0) {
                    new obsidian_1.Notice('All files already have namespace metadata.');
                    return;
                }
                // Show confirmation modal
                new BulkOperationModal(this.app, plannedChanges.map(c => c.file), () => this.executeAtomicOperation(plannedChanges), () => new obsidian_1.Notice('Operation cancelled.')).open();
            }
            catch (error) {
                console.error('Error during file processing:', error);
                new obsidian_1.Notice(`❌ Error: ${(error === null || error === void 0 ? void 0 : error.message) || 'Unknown error'}`);
            }
        });
    }
    /**
     ...
     */
    executeAtomicOperation(plannedChanges) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                backups = yield this.createBackups(filesToModify);
                ...
                for (let i = 0; i < plannedChanges.length; i++) {
                    ...
                    try {
                        yield this.app.vault.modify(change.file, change.newContent);
                        progress.update();
                        ...
                        throw new Error(`Failed to modify ${change.file.path}: ${(error === null || error === void 0 ? void 0 : error.message) || 'Unknown error'}`);
                    }
                }
                ...
                if (backups.length > 0) {
                    ...
                    yield this.restoreFromBackups(backups);
                    progress.finish('❌ Operation failed. All files restored to original state.');
                }
                new obsidian_1.Notice(`❌ Operation failed: ${(error === null || error === void 0 ? void 0 : error.message) || 'Unknown error'}\nAll files have been restored.`);
            }
        });
    }
    /**
...
exports.default = LogseqNamespaceInjector;
/**
 * Enhanced settings tab with all configuration options
 */
class LogseqNamespaceInjectorSettingTab extends obsidian_1.PluginSettingTab {
